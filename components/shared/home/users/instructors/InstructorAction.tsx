...
export default function InstructorAction({ id }: { id: number }) {
  const router = useRouter();
  const { data: session } = useSession();
  const { setEditID, toggleInstructorEdit } = useEditStore();

  const handleDelete = () => {
    const isConfirm = confirm("Are you sure you want to delete this instructor?");
    if (!isConfirm) return;
    axiosInstance
      .delete(`/private/instructor/delete/${id}`, {
        headers: {
