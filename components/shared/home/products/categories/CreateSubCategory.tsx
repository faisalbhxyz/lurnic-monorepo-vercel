...
export default function CreateSubCategory({
  ...
  const handleSave = (data: TCategorySchema) => {
    if (isEdit) {
      axiosInstance
        .put(`/private/subcategory/update/${subcategory?.id}`, data, {
          headers: {
      ...
      axiosInstance
        .post("/private/subcategory/create", data, {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${session?.accessToken}`,
          },
      ...
      <div className="flex-between px-5 py-3 bg-white border-b border-gray-300">
        <div className="w-full flex items-center justify-between gap-5">
          <h3 className="font-medium">
            {isEdit ? "Edit" : "Create"} Sub Category
          </h3>
            ...
            <Controller
              ...
              render={({ field: { onChange, value } }) => (
                <GenericSelectWithSearch
                  items={categories || []}
                  selectedItem={
                    categories && categories.find((cat) => cat.id === value)
                  }
