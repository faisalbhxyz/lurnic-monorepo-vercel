...
export default function SubCategoryAction({ id }: { id: number }) {
  const router = useRouter();
  const { data: session } = useSession();

  const handleDelete = () => {
    const isConfirm = confirm("Are you sure you want to delete this sub category?");
    if (!isConfirm) return;
    axiosInstance
      .delete(`/private/subcategory/delete/${id}`, {
        headers: {
      ...
      >
        <MenuItem>
          <button
            ...
            onClick={() => router.push(`/subcategory/${id}/edit`)}
          >
            <BiEditAlt size={18} />
            Edit
          </button>
