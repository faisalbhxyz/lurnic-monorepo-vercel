...
export default function PaymentAction({ id }: { id: number }) {
  const router = useRouter();
  const { data: session } = useSession();
  const { refreshPaymentMethods, setEditID, togglePaymentMethodEdit } =
    useEditStore();
  ...
  const handleDelete = () => {
    ...
    if (!isConfirm) return;
    axiosInstance
      .delete(`/private/payment-method/delete/${id}`, {
        headers: {
